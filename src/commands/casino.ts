...
export default new AmethystCommand({
    ...
    setTimeout(() => {
        ...
        const msg = (lastGenerated.raw[3].emoji === lastGenerated.raw[4].emoji === lastGenerated.raw[5].emoji) ? `💲 Vous avez gagné **${mise * signs.find(x => x.emoji === lastGenerated.raw[3].emoji).times}** 🪙` : `Vous avez perdu **${mise}** 🪙`
        interaction.editReply({
            content: `🎰 __**Casino**__\n\n${lastGenerated.text}\n${msg}`
        }).catch(() => {});
    }, 3000)
