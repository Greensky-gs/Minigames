...
export default new AmethystCommand({
    ...
    const msg = await interaction.reply({
        embeds: [
            basicEmbed(user)
                .setTitle("Morpion")
                .setDescription(`<@${interaction.user.id}> vous défie dans une partie de morpion sans merci, relevez-vous le défi ?`)
                .setColor('Orange')
        ],
        content: `<@${user.id}>`,
    ...
    if (!confirmation || confirmation.customId === 'no') return interaction.editReply({
        embeds: [ canceled() ],
        components: []
    }).catch(() => {});
